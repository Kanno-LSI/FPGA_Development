...
### ハザードマップの合成
今回のシステムでは32×32地点の区域の計算を行っていますが、実際のハザードマップはより広い地域にする必要があります。そこで、複数区域を計算してからそれらを合成することで、さらなる広い区域のハザードマップを作成することができるようにしました。以下の図は津波の予測を行い、3×3区域の合成を行ったハザードマップです。
